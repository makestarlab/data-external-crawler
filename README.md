...
**프롬프트에 트윗이 안 들어갔다** — 같은 날 세 번째 사고. 1,749건 중 192행만 남고
그 192행은 전부 `is_relevant=false` 에 필드가 모두 비어 있었다.

원인은 `call_claude` 안의 **변수 섀도잉**이었다.

```python
lines = []                                   # 트윗 목록
for t in tweets: lines.append(...)

if artist_roster:
    lines = [ln for ln in artist_roster...]  # 로스터로 덮어씀

user_msg = (... + "\n".join(lines))          # 로스터가 들어가고 트윗은 사라짐
```

모델은 정직하게 답했다. `note` 컬럼에 전부 이렇게 적혀 있었다.

```
트윗 본문 없이 아티스트명만 제공되어 판단 불가
실제 트윗 본문이 제공되지 않아 판정 불가
```

**결과가 이상하면 모델이 남긴 note 를 먼저 읽을 것.** 이 사고는 로그를 세 번 뒤지는 것보다
`SELECT note FROM x_tour_announcements LIMIT 5` 한 번이 빨랐다.

고친 방식:

- 메시지 조립을 `build_user_message()` 로 분리했다. 함수로 빼면 "메시지에 본문이 들어 있는가"
  를 테스트로 못 박을 수 있다 (회귀 테스트 10건 추가)
- 로스터를 아예 안 넣는다. `curate_tour` 는 ARTIST 계정만 처리하고 계정 주인은 이미 아는
  값이라, 수백 줄 명단 대신 그 아티스트 이름 한 줄이면 충분하다.
  프롬프트가 438자로 짧아졌고 섀도잉의 원인 자체가 사라졌다
- 포스팅 목록을 메시지 **맨 끝**에 고정했다. 중간에 다른 목록을 끼우면 "아래 목록" 이
  무엇을 가리키는지 모호해진다

사고로 들어간 192행은 `sql/cleanup_empty_tour_rows_20260901.sql` 로 지운다.
**안 지우면 안티조인 때문에 그 트윗들이 영원히 재처리에서 빠진다.**

### 2026-09-02 첫 정상 실행에서 나온 조정

7일치 201건이 유실 0으로 처리됐다 (투어 공지 38건 · 회차 29개). 그 결과를 보고 세 가지를 고쳤다.

**확인 필요 66% 는 판정 규칙의 오탐이었다.** 25건 중 13건이 "일정 공지인데 확정 날짜 없음"
이었는데, `NEW_TOUR`(투어 최초 발표)는 "월드투어 갑니다, 도시는 추후 공개" 형태라 날짜가
없는 게 정상이고 `TICKET_OPEN`도 이미 발표된 공연의 예매 안내라 본문에 날짜가 없을 수 있다.
두 유형을 이 규칙과 "도시 결측" 규칙에서 뺐다.

**표기 흔들림.** 실측에서 나온 것들:

| 증상 | 조치 |
|---|---|
| 아티스트명이 한글 (`스트레이 키즈`) | `entity_master.name_en` 을 프롬프트에 준다. 시트 IP 컬럼이 영문이라 한글이면 조인이 깨진다 |
| `SOUTH KOREA` / `KOREA` 혼재 | `normalize_country()` — 시트 표기로 모은다 |
| 같은 공지의 한국어판·영어판이 `서울`/`Seoul` 두 행 | `normalize_city()` + `show_key` 를 정규화된 도시로 생성 |
| 전시회·패션쇼 유입 (NCT 전시, 도쿄 걸즈 컬렉션) | 판정 기준에 제외 항목 명시. 팀이 보는 건 **공연하러 오는** 시점이다 |

**중간 취소로 74회 호출분이 날아갔다.** 적재가 루프 끝에 한 번뿐이라 그렇다.
이제 `FLUSH_EVERY`(기본 40행)마다 중간 적재하고, `TOUR_MAX_MINUTES`(기본 150분)에
스스로 멈춰 남은 결과를 적재한 뒤 나간다. 러너 타임아웃은 60 → 180분으로 올렸다
(러너가 죽이면 적재 기회 자체가 없다).

백필 소요 시간 실측 — 호출당 약 17초 기준:

| lookback | 포스팅 | 예상 호출 | 예상 시간 |
|---|---|---|---|
| 7일 | 201 | 42 | 12분 |
| 30일 | 849 | 102 | 29분 |
| 60일 | 1,707 | 174 | 49분 |
...
### 확인 필요(needs_review) 판정 기준을 정한 근거

30일치 82건의 사유 분포와 confidence 분포를 보고 정했다.

| confidence | 건수 | 전 회차 날짜 | 전 회차 도시 | 전 회차 베뉴 |
|---|---|---|---|---|
| 0.90+ | 25 | 25 | 25 | 25 |
| 0.80~0.89 | 8 | 4 | 6 | 5 |
| **0.70~0.79** | **17** | **14** | **14** | **2** |
| 0.60~0.69 | 14 | 6 | 13 | 3 |
| 0.50~0.59 | 14 | 1 | 10 | 1 |
| <0.50 | 3 | 0 | 2 | 0 |

**0.70~0.79 구간 17건이 핵심이다.** 날짜와 도시는 멀쩡히 다 있고 베뉴만 없다.
노션에 적힌 그대로 "투어 발표가 공연장 확정보다 먼저" 인 정상 상태다.
문턱을 0.75 로 두면 이 17건이 통째로 확인 큐로 갔다.

그래서 두 가지를 바꿨다.

- **공연장 결측을 확인 필요에서 제외.** 확인 필요 49건 중 29건이 이 사유였는데,
  사람이 손댈 게 없는 건이다. 사람이 손댈 게 없는 걸 큐에 쌓으면 큐를 안 보게 된다.
  베뉴 미정 회차는 `v_tour_venue_pending` 뷰로 따로 본다 — 나중에 Ticketmaster 조회나
  후속 공지로 채울 대상이지, 지금 확인할 대상이 아니다
- **confidence 문턱 0.75 → 0.5** (`TOUR_CONF_THRESHOLD`). 0.50~0.59 부터 실제로
  정보가 부실해진다(전 회차 날짜 보유 1/14). 확신도는 보조 신호로만 쓰고 판정은 내용이 한다

남긴 규칙은 팀이 실제로 손댈 수 있는 것들이다 — 날짜 결측, 도시 결측,
`entity_master` 매칭 실패, 날짜 파싱 실패, confidence 0.5 미만.

### v_tour_shows_latest 가 조회 시점에 깨져 있었다
...
중복 제거 뷰가 계속 작동하지 않고 있었다. 창을 쓰지 말고 일반 집계로 먼저 접은 뒤
조인하도록 고쳤다 (`sql/fix_tour_views_20260902.sql`).

> 뷰를 만들거나 고치면 **반드시 한 번 SELECT 해볼 것.** DDL 성공은 동작 보증이 아니다.
