...
프롬프트에 함께 주어지는 **"이미 등록된 최근 이벤트 목록"** 에서 같은 계정의 같은 이벤트를 찾아라.
찾으면 `is_relevant=true`, 그 `event_key` 를 그대로 재사용하고, 목록에 있는
`artist_name` · `album_or_title` · `seller_name` 을 그대로 채운다. 세 질문은 건너뛴다.
목록에서 못 찾으면 그때는 제외한다.
